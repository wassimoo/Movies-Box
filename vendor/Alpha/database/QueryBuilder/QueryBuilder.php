...
// TODO : add 'on' condition support alongside 'where' and 'having'
...
class QueryBuilder implements BuilderInterface
{
    ...
    /**
     * @var array  working tables
     */
    private $tables;

    /**
     * @var array columns being projected.
     */
    private $projectionColumns;
    /**
     * @var array tmp restriction columns used in where() | having()
     */
    private $restrictionColumns;
    ...
    /**
     * @var bool not() method has been called  ?
     */
    private $notFlag;

    /**
     * @var string final where condition generated.
     */
    private $generatedWhereCondition;
    /**
     * @var string final having condition generated.
     */
    private $generatedHavingCondition; //


    /**
     * @var string last called function , used to differentiate where / having / on conditions
     */
    private $lastCalledFunc;

    /**
     * @var string final generated query
     */
    private $query;
    ...
    private function buildQuery()
    {
        if ($this->query == "SELECT") {
            ...
            if(!empty($this->conditions)){
                ...
                if($this->generatedWhereCondition != "")
                    $this->query .= " WHERE " . $this->generatedWhereCondition;
                if($this->generatedHavingCondition != "")
                    $this->query .= " WHERE " . $this->generatedHavingCondition;
            }
        }
    }
    ...
    {
        /* if $conditions is not empty => generate condition and add or , 
            otherwise 
            
        /* append 'OR' to condition */
        if (!empty($this->conditions)) {
            $this->conditions[count($this->conditions)-1] .= " $bool ";
        } else {
...
/*********************************************************************************************/
    ...
    {
        $expr = $this->notFlag ? " NOT BETWEEN " : " BETWEEN ";
        foreach ($this->restrictionColumns as $column) {
            $this->conditions[] = $column . $expr . "$a and $b ";
        }
